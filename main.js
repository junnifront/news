...
const getNewsByTopic = (event) => {
  ...
  page = 1;
  url = new URL(
    `https://newsapi.org/v2/top-headlines?country=us&&pageSize=${PAGE_SIZE}&category=${topic}&apiKey=${API_KEY}`
  );
  getNews();
};
...
const getNewsByKeyword = () => {
  ...
  page = 1;
  url = new URL(
    `https://newsapi.org/v2/top-headlines?country=us&&pageSize=${PAGE_SIZE}&q=${keyword}&apiKey=${API_KEY}`
  );
  getNews();
};
